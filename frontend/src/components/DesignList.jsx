...
const DesignList = ({ items, onEdit, onDeleted, onReorder }) => {
  ...
  const handleDelete = async (slug) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/design/${slug}`, {
        method: "DELETE",
        credentials: "include",
      });
  ...
  if (localItems.length === 0) {
    return <p className="text-sm text-muted mt-6">No designs yet.</p>;
  }
